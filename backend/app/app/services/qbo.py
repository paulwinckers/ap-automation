...
class QBOClient:
    ...
    async def find_account(self, account_code: str) -> Optional[dict]:
        """
        Look up a QBO account by its AcctNum (GL code).
        e.g. find_account("6200") returns the Office Supplies account.
        """
        result = await self._get(
            "query",
    ...
    async def post_bill(self, invoice: Invoice, gl_account: str) -> str:
        """
        Create a Bill in QBO for an overhead invoice.

        gl_account: the GL account code from vendor_rules.default_gl_account
                    e.g. "6200" (maps to Office Supplies in QBO chart of accounts)

        Returns the QBO Bill Id.

        Bill structure:
          - Header: vendor ref, dates, currency CAD
          - Lines: one AccountBasedExpenseLine per invoice line item
                   each line references the GL account + GST/PST tax code
          - TxnTaxDetail: QBO calculates tax from the line tax codes
        """
        # ── Resolve vendor ────────────────────────────────────────────────────
        ...
        if invoice.line_items:
            ...
            # Fallback: single line for the subtotal if no line items extracted
            lines = [
                {
        ...
        bill_body = {
            ...
            "Line": lines,
            # Let QBO calculate tax from line tax codes
