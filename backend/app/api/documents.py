...
async def upload_document(
    ...
    logger.info(f"Document uploaded to R2: {r2_key}")

    # Insert into DB
    rows = await db._q(
        "INSERT INTO company_documents (title, description, r2_key, filename, file_size, uploaded_by) "
        "VALUES (?, ?, ?, ?, ?, ?) RETURNING id, created_at",
        [title.strip(), (description or "").strip() or None, r2_key, filename, len(file_bytes), uploaded_by or "Admin"],
    )
    doc_id = rows[0]["id"] if rows else None
    logger.info(f"Document #{doc_id} '{title}' saved by {uploaded_by}")
...
async def get_document_file(doc_id: int, db: Database = Depends(get_db)):
...
async def delete_document(doc_id: int, db: Database = Depends(get_db)):
    """Soft-delete a document (admin)."""
    await db._x(
        "UPDATE company_documents SET is_active = 0 WHERE id = ?",
        [doc_id],
    )
    logger.info(f"Document #{doc_id} soft-deleted")
